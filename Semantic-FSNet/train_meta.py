...
def main(config):
    ...
    if config.get('load'):
        ...
        model = models.load(model_sv, args=args)
    else:
        model = models.make(config['model'], args, **config['model_args'])
    ...
    for epoch in range(1, max_epoch + 1):
        ...
        for data, _ in tqdm(train_loader, desc='train', leave=False):
            ...
            indices = torch.tensor(range(n_train_shot))
            labels = torch.index_select(labels, 1, indices)
            labels = labels.view(-1)

            logits, score = model(epoch, labels.long(), x_shot, x_query)
            logits = logits.view(-1, n_train_way)
            if epoch > args.epoch:
                loss = F.cross_entropy(logits, label)
            else:
                loss = args.w * F.cross_entropy(logits, label) + (1 - args.w) * score
...
if __name__ == '__main__':
    ...
    parser.add_argument('--freeze_param', action='store_true', help='Freeze parameters')
